// tests/unit/contexts/financial-tracking/http/delete-tag.route.test.ts
...
describe("DELETE /families/:familyId/categories/:categoryId/tags/:tagId", () => {
  ...
  test("elimina un tag sin items cuando lo solicita un Owner", async () => {
    const response = await app.inject({
      method: "DELETE",
      url: `/families/${familyId}/categories/${categoryId}/tags/${tagId}`,
  ...
  test("rechaza eliminar un tag con items asociados", async () => {
    await financialItemRepository.save(
      FinancialItem.create({
        familyId: FamilyId.of(familyId),
        recordedBy: ownerId,
        amount: Money.of(5000, "CLP"),
        category: CategoryAssignment.of(CategoryId.of(categoryId), TagId.of(tagId)),
    ...
    assert.equal(JSON.parse(response.body).error, "FINANCIAL_TRACKING.TAG_HAS_ASSOCIATED_ITEMS");
  });
