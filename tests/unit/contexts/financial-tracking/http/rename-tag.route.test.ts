// tests/unit/contexts/financial-tracking/http/rename-tag.route.test.ts
...
describe("PATCH /families/:familyId/categories/:categoryId/tags/:tagId", () => {
  ...
  test("renombra un tag cuando lo solicita un Owner", async () => {
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${familyId}/categories/${categoryId}/tags/${tagId}`,
      headers: { authorization: ownerAuthorization },
      payload: { name: "Almacén" },
    });

    assert.equal(response.statusCode, 200);
    const body = JSON.parse(response.body);
    assert.equal(body.id, tagId);
    assert.equal(body.name, "Almacén");
  ...
  test("rechaza un nombre de tag duplicado", async () => {
    const createResponse = await app.inject({
      method: "POST",
      url: `/families/${familyId}/categories/${categoryId}/tags`,
      headers: { authorization: ownerAuthorization },
      payload: { name: "Verdulería" },
    });
