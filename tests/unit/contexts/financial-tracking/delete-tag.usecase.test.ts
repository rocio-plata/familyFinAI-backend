// tests/unit/contexts/financial-tracking/delete-tag.usecase.test.ts
...
import { DeleteTagUseCase } from "../../../../src/contexts/financial-tracking/application/commands/delete-tag.usecase.js";
...
import { TagHasAssociatedItemsError } from "../../../../src/contexts/financial-tracking/domain/errors/tag-has-associated-items.error.js";
import { TagNotFoundError } from "../../../../src/contexts/financial-tracking/domain/errors/tag-not-found.error.js";
import { TagDeletionService } from "../../../../src/contexts/financial-tracking/domain/services/tag-deletion.service.js";
...
describe("DeleteTagUseCase", () => {
  ...
  beforeEach(async () => {
    ...
    useCase = new DeleteTagUseCase(
      categoryRepository,
      new TagDeletionService(financialItemRepository),
  ...
  test("elimina el tag cuando lo solicita un Owner y no tiene items", async () => {
  ...
  test("rechaza si el tag tiene items asociados", async () => {
    ...
    await assert.rejects(
      ...
      TagHasAssociatedItemsError,
    );
  });

  test("no elimina el tag si tiene items asociados", async () => {
    const item = FinancialItem.create({
      familyId,
      recordedBy: ownerId,
      amount: Money.of(5000, "CLP"),
    ...
    await assert.rejects(() =>
      useCase.execute({ familyId, requestedBy: ownerId, categoryId: category.id, tagId }),
    );
