// tests/unit/contexts/family-access/http/change-member-role.route.test.ts
...
describe("PATCH /families/:familyId/members/:memberId/role", () => {
  ...
  test("cambia el rol de un miembro cuando quien ejecuta es Owner", async () => {
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${family.id.toString()}/members/${memberId.toString()}/role`,
  ...
  test("rechaza con 403 si quien ejecuta no es Owner", async () => {
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${family.id.toString()}/members/${targetId.toString()}/role`,
      headers: { authorization: `Bearer ${token}` },
      payload: { newRole: "OWNER" },
    });
  ...
  test("devuelve 400 si se intenta degradar al último Owner a Member", async () => {
    const ownerId = UserId.generate();
    const family = Family.create(FamilyName.of("Familia Pérez"), ownerId);
    await familyRepository.save(family);
    const token = await jwtService.sign(ownerId);
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${family.id.toString()}/members/${ownerId.toString()}/role`,
      headers: { authorization: `Bearer ${token}` },
  ...
  test("devuelve 404 si el miembro objetivo no existe en la familia", async () => {
    const ownerId = UserId.generate();
    const family = Family.create(FamilyName.of("Familia Pérez"), ownerId);
    await familyRepository.save(family);
    const token = await jwtService.sign(ownerId);
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${family.id.toString()}/members/${UserId.generate().toString()}/role`,
      headers: { authorization: `Bearer ${token}` },
      payload: { newRole: "OWNER" },
    });
  ...
  test("rechaza un identificador de miembro inválido", async () => {
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${family.id.toString()}/members/no-es-un-uuid/role`,
      headers: { authorization: `Bearer ${token}` },
      payload: { newRole: "OWNER" },
    });
  ...
  test("rechaza un newRole fuera del enum permitido", async () => {
    const ownerId = UserId.generate();
    const memberId = UserId.generate();
    const family = Family.create(FamilyName.of("Familia Pérez"), ownerId);
    family.addMemberFromInvitationData(memberId, Role.member());
  ...
  test("rechaza con 403 si la familia no existe (mismo status que 'no soy miembro')", async () => {
    ...
    const response = await app.inject({
      method: "PATCH",
      url: `/families/${FamilyId.generate().toString()}/members/${UserId.generate().toString()}/role`,
      headers: { authorization: `Bearer ${token}` },
      payload: { newRole: "OWNER" },
    });
