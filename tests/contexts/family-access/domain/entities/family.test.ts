...
function addMember(family: Family, role: Role): UserId {
  ...
  const inv = family.inviteMember(EmailAddress.of(`${newUserId.toString().slice(0, 8)}@test.com`), role);
...
describe('Family', () => {
  ...
  describe('removeMember()', () => {
    ...
    it('lanza InsufficientRoleError si quien elimina no es owner', () => {
      ...
      assert.throws(() => family.removeMember(memberId, otherMember), { name: 'InsufficientRoleError' });
    });

    it('lanza CannotRemoveLastOwnerError al intentar eliminar el único owner', () => {
      const { family, creator } = makeFamily();
      assert.throws(() => family.removeMember(creator, creator), { name: 'CannotRemoveLastOwnerError' });
    });
  });
  ...
  describe('changeRole()', () => {
    ...
    it('lanza CannotRemoveLastOwnerError al degradar al único owner', () => {
      const { family, creator } = makeFamily();
      assert.throws(() => family.changeRole(creator, Role.member()), { name: 'CannotRemoveLastOwnerError' });
    });

    it('lanza MemberNotFoundError para un userId inexistente', () => {
      const { family } = makeFamily();
      assert.throws(() => family.changeRole(UserId.generate(), Role.member()), { name: 'MemberNotFoundError' });
    });
  });
});
