...
4. **Testing de integración contra Postgres real**: ✅ resuelto localmente sin nuevas librerías de testing. `tests/integration/` usa `node:test` y la conexión Drizzle/`pg` existente; `tests/e2e/` usa `buildApp()` + `app.inject()`. Ambas suites se ejecutan con scripts separados y requieren `DATABASE_URL`. Queda pendiente automatizarlas en CI.
