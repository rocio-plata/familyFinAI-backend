...
5. **`CreatePaymentMethodUseCase`, `RenamePaymentMethodUseCase`, `GetPaymentMethodsQuery`** — `CreatePaymentMethodUseCase` ✅ implementado en `src/contexts/financial-tracking/application/commands/create-payment-method.usecase.ts`, con tests. Está scoped por `familyId`, permite cualquier `Member` y aplica unicidad case-insensitive dentro de la familia. `RenamePaymentMethodUseCase` y `GetPaymentMethodsQuery` siguen pendientes.
...
13. **`CreatePaymentMethodUseCase`, `RenamePaymentMethodUseCase`, `GetPaymentMethodsQuery`** — `CreatePaymentMethodUseCase` ✅ implementado con tests TDD. `RenamePaymentMethodUseCase` y `GetPaymentMethodsQuery` siguen pendientes.
