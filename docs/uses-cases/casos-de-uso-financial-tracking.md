...
> **Estado de implementación**: los 17 casos de uso de este documento (1–25) ya están implementados en `src/contexts/financial-tracking/application/`, con sus tests correspondientes en `tests/contexts/financial-tracking/`. Las secciones de "Errores" y "Pendientes" al final de este documento reflejan las decisiones ya tomadas durante la implementación.
...
### 19. OnInvitationAccepted

Establece automáticamente la preferencia inicial de un miembro cuando acepta una invitación.

- **Actor**: handler interno de `Financial Tracking`.
- **Entrada**: evento `InvitationAccepted` con `familyId` y `acceptedBy`.
- **Flujo principal**:
...
Elimina la preferencia de medio de pago de un miembro cuando deja una familia.

- **Actor**: handler interno de `Financial Tracking`.
- **Entrada**: evento `MemberRemoved` con `familyId` y `removedUserId`.
- **Flujo principal**:
  1. Se recibe el evento publicado por `Family & Access`.
...
Renombra un medio de pago compartido dentro de una familia.

- **Actor**: cualquier `Member` de la familia.
- **Entrada**: `familyId`, `requestedBy`, `paymentMethodId`, `newName`.
- **Flujo principal**:
  1. Se valida que quien solicita pertenezca a la familia.
  2. Se busca el medio y se valida que pertenezca a la familia solicitada.
  3. Se verifica que el nuevo nombre no esté usado por otro medio de la familia, sin distinguir mayúsculas.
...
Lista los medios de pago de una familia.

- **Actor**: cualquier `Member` de la familia, con autorización resuelta por la capa HTTP.
- **Entrada**: `familyId`, `includeDeprecated` opcional (por defecto `false`).
- **Flujo principal**:
  1. Se consultan los medios de pago de la familia.
  2. Se excluyen los deprecados por defecto.
...
Marca un medio de pago como deprecado sin eliminarlo, preservando su referencia histórica.

- **Actor**: cualquier `Member` de la familia.
- **Entrada**: `familyId`, `requestedBy`, `paymentMethodId`.
- **Flujo principal**:
  1. Se valida que quien solicita pertenezca a la familia.
  2. Se busca el medio y se valida que pertenezca a la familia solicitada.
  3. Se verifica que ninguna preferencia de usuario lo use como default.
...
Elimina físicamente un medio de pago que ya no tiene referencias.
...
- **Flujo principal**:
  1. Se valida que quien solicita pertenezca a la familia.
  2. Se busca el medio y se valida que pertenezca a la familia solicitada.
  3. Se verifica que ninguna preferencia lo use como default.
  4. `PaymentMethodDeletionService` verifica que no tenga items asociados.
...
> Nota: los puntos 1–5 quedaron resueltos durante la implementación de los 25 casos de uso; se dejan documentados como registro de la decisión tomada. El punto 6 sigue abierto.
