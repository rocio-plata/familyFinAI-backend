...
> unitarias, persistencia, composición y ruta HTTP implementadas.
...
> para conservar el histórico. La persistencia, composición y ruta HTTP están implementadas.
...
> gastos e ingresos. Para una base de comparación en cero, la variación porcentual es `0`. La
...
> admite filtro por `categoryId` y lanza `InvalidPeriodRangeError` para rangos invertidos. La
> persistencia, composición y ruta HTTP están implementadas.
...
> individuales filtrados por categoría y tag. La persistencia, composición y ruta HTTP están
> implementadas.
...
> La persistencia y suscripción en la composición de la aplicación están implementadas.
...
> aplica la diferencia entre `previousAmount` y `newAmount` sin modificar `itemCount`. La
> persistencia y suscripción en la composición de la aplicación están implementadas.
...
> falta. La persistencia y suscripción en la composición de la aplicación están implementadas.
...
> resta el movimiento del agregado de familia, categoría y período correspondiente. La persistencia
> y suscripción en la composición de la aplicación están implementadas.
...
|---|---|---|
| `InvalidPeriodRangeError` | GetTrend | ✅ definido e implementado |
| `TagWithoutCategoryError` | GetDrillDown | ✅ definido e implementado |
