...
- **Financial Tracking**: casos de uso y entidades/value objects centrales implementados; `POST /families/:familyId/categories` ✅, `GET /families/:familyId/categories` ✅, `POST /families/:familyId/categories/:categoryId/tags` ✅, `PATCH /families/:familyId/categories/:categoryId` ✅, `PATCH /families/:familyId/categories/:categoryId/tags/:tagId` ✅, `PUT /families/:familyId/categories/:categoryId/tags/order` ✅, `POST /families/:familyId/categories/:categoryId/deprecate` ✅, `POST /families/:familyId/categories/:categoryId/tags/:tagId/deprecate` ✅, `DELETE /families/:familyId/categories/:categoryId` ✅, `DELETE /families/:familyId/categories/:categoryId/tags/:tagId` ✅, `POST /families/:familyId/items` ✅, `GET /families/:familyId/items` ✅, `PATCH /families/:familyId/items/:itemId` ✅, `PATCH /families/:familyId/items/:itemId/category` ✅ y `DELETE /families/:familyId/items/:itemId` ✅ son los endpoints expuestos hasta ahora. **Budgeting, Reporting y AI Assistance**: documentados, con sus entidades/value objects centrales implementados, pero sin rutas HTTP todavía.
...
12. ~~`GET /families/:familyId/items` (`GetFinancialItems`)~~ ✅ — requiere membresía de la familia y admite filtros opcionales por `from`/`to`, `categoryId`, `tagId` y `type`.
13. ~~`PATCH /families/:familyId/items/:itemId` (`UpdateFinancialItem`)~~ ✅ — requiere membresía de la familia; permite cambiar monto, fecha, título y nota, pero rechaza cambios de categoría, tag o tipo.
14. ~~`PATCH /families/:familyId/items/:itemId/category` (`ReclassifyFinancialItem`)~~ ✅ — requiere membresía de la familia; recibe una nueva categoría activa y un tag opcional que debe pertenecerle.
15. ~~`DELETE /families/:familyId/items/:itemId` (`DeleteFinancialItem`)~~ ✅ — requiere membresía de la familia, elimina físicamente el movimiento y publica `ItemDeleted`.
