...
class DeprecateTagUseCase {
  ...
  async execute(input: DeprecateTagInput): Promise<Category> {
    // 1. Solo un Owner puede deprecar tags
    ...
    // 3. Deprecar el tag (siempre permitido, sin chequeo de items; lanza TagNotFoundError si no existe)
    category.deprecateTag(input.tagId);
