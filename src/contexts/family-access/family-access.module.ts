...
function buildFamilyAccessModule(deps: FamilyAccessModuleDependencies): FamilyAccessModule {
  const useCases = {
    ...
    inviteMember: new InviteMemberUseCase(deps.familyRepository, deps.invitationRepository, deps.userDirectory, deps.eventBus),
    acceptInvitation: new AcceptInvitationUseCase(deps.familyRepository, deps.invitationRepository, deps.eventBus),
  ...
  return {
    ...
    authenticate,
    requireFamilyMembership: (minRole) => requireFamilyMembership(useCases.getFamilyMembership, minRole),
